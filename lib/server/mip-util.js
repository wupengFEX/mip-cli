 ...
 * @return {boolean} 是否是 mip 域名
 */
function checkValidDomain (url) {
    if (!url) {
        return false;
    }
    if (url.indexOf('mipcache.bdstatic.com') < 0
        && url.indexOf('c.mipcdn.com') < 0) {
        return false;
    }
    return true;
}
...
exports.pageCustomInject = function (content) {
    ...
    return content.replace(regex, ($0, script, src) => {
        if (src.indexOf('mipcache.bdstatic.com/static/v1/mip-custom') < 0
            && src.indexOf('c.mipcdn.com/static/v1/mip-custom') < 0) {
            return $0;
        }
...
exports.mipmaiInject = function (content) {
    ...
    content = content.replace(regexJs, ($0, header, src, footer) => {
        var mipJs;
        ...
        if (mipJs) {
            return header + '/miplocal/dist/' + mipJs + footer;
        }
