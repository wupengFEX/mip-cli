...
const url = 'https://c.mipcdn.com/static/v1/mip-custom/mip-custom.js';
...
function startServer(config) {
    ...
    if (config.mipCustomDir) {
        app.use('/mipcustom', function (req, res) {
            if (req.path.indexOf('/common') >= 0) {
                customPreview.exec(config, function (data) {
                    res.json(data);
                });
            } else {
                var pth = path.join(config.baseDir, config.mipCustomDir , '/dist', req.path);
                res.sendFile(pth);
            }
        });
